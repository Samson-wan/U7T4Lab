...
public class ArrayListAlgorithms {
    ...
    public static void duplicateUpperAfter(ArrayList<String> wordList){
        for(int i = 0; i < wordList.size(); i++){
            wordList.add(i + 1, wordList.get(i).toUpperCase(Locale.ROOT));
            i++;
        }
    }
    ...
    public static void duplicateUpperEnd(ArrayList<String> wordList){
        ...
        for(int i = 0; i < size; i++){
            wordList.add(wordList.size(), wordList.get(i).toUpperCase(Locale.ROOT));
        }
    }
    ...
    public static ArrayList<String> parseWordsAndReverse(String sentence){
        ...
        for(int i = 0; i < sentence.length(); i++){
            if(sentence.substring(i, i + 1).equals(" ")){
                reverse.add(0, sentence.substring(index, i));
                index = i + 1;
            }
        }
        reverse.add(0, sentence.substring(index, sentence.length()));
        return reverse;
    }
    ...
    public static ArrayList<Integer> modes(int[] numList){
        ...
        int currentMode = Integer.MIN_VALUE;
        int modeCount = 1;
        for(int i = 0; i < numList.length; i++){
            int count = 1;
            if(numList[i] != currentMode){
            ...
            else if(count == modeCount && count != 1){
                list.add(numList[i]);
            }
        }
        return list;
